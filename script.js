...
const SCORE_LIMITS = {
    good: 10,      // < 10 kg = Baik
    moderate: 20,  // 10-20 kg = Sedang  
    high: 30,      // 20-30 kg = Tinggi
    veryHigh: Infinity // > 30 kg = Sangat Tinggi
};
...
function calculateCarbonFootprint(e) {
    ...
    // Calculate emissions
    const emissions = {
        transport: (inputs.carDistance * EMISSION_FACTORS.car) +
                   (inputs.motorDistance * EMISSION_FACTORS.motorcycle) +
                   (inputs.publicTransport * EMISSION_FACTORS.publicTransport),
        
        energy: (inputs.electricity * EMISSION_FACTORS.electricity) +
                (inputs.gas * EMISSION_FACTORS.gas),
        
        food: (inputs.redMeat * EMISSION_FACTORS.redMeat) +
...
function updateScoreCircle(total) {
    ...
    if (total < SCORE_LIMITS.good) color = '#4CAF50';
    else if (total < SCORE_LIMITS.moderate) color = '#8BC34A';
    else if (total < SCORE_LIMITS.high) color = '#FF9800';
    else color = '#F44336';
...
function updateTips(emissions) {
    ...
    if (emissions.transport > 0) {
        tipsHtml += generateTipsHTML(TIPS.transport, 'fas fa-car', '#FF6B6B');
    }
    if (emissions.energy > 0) {
        tipsHtml += generateTipsHTML(TIPS.energy, 'fas fa-bolt', '#4ECDC4');
    }
    if (emissions.food > 0) {
        tipsHtml += generateTipsHTML(TIPS.food, 'fas fa-utensils', '#45B7D1');
    }
    
    // Add general tips
    tipsHtml += generateTipsHTML(TIPS.general, 'fas fa-leaf', '#96CEB4');
    
    tipsList.innerHTML = tipsHtml;
}

// Generate tips HTML
function generateTipsHTML(tips, icon, color) {
    let html = `<div class="tips-category">
        <div class="tips-header" style="border-left-color: ${color};">
            <i class="${icon}" style="color: ${color};"></i>
            <span>Tips Khusus</span>
        </div>
...
function resetCalculator() {
    ...
    // Scroll to form
    document.querySelector('.calc-form').scrollIntoView({ behavior: 'smooth' });
}
...
// Add input helper text on focus
...
// Add CSS for animations (inline)
const style = document.createElement('style');
style.textContent = `
    .form-group, .results, .tips {
        opacity: 0;
        transform: translateY(30px);
        transition: all 0.6s ease;
    }
    
    .form-group.animate, .results.animate, .tips.animate {
        opacity: 1;
...
document.addEventListener('DOMContentLoaded', function() {
    // Pre-fill with example data (optional)
    // document.getElementById('electricity').value = '20';
    // document.getElementById('motorDistance').value = '50';
